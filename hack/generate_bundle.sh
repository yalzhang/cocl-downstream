...
CSV_TEMPLATE="${PROJECT_ROOT}/bundle/static/manifests/cocl-operator.clusterserviceversion.yaml"
ANNOTATIONS_TEMPLATE="${PROJECT_ROOT}/bundle/static/metadata/annotations.yaml"
RBAC_ROLE_FILE="${PROJECT_ROOT}/operator/config/rbac/role.yaml"
...
shopt -s nullglob
cp "${PROJECT_ROOT}/operator/config/crd"/*.yaml "${BUNDLE_MANIFESTS}/"
cp "${PROJECT_ROOT}/operator/config/rbac"/*.yaml "${BUNDLE_MANIFESTS}/"
...
yq -i ".metadata.name = \"cocl-operator.v${BUNDLE_VERSION}\"" "$CSV_FILE"
